...
export default function AllogramApp() {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      if (savedWall) { try { const parsed = JSON.parse(savedWall); if (parsed && (parsed.bgImage || parsed.bgColor)) setWallpaper(parsed); } catch (e) { setWallpaper(DEFAULT_WALLPAPER); } }
  ...
  const WallpaperPreviewEditor = () => {
    ...
    const handlePointerDown = (e) => { if (mode !== 'custom') return; const clientX = e.clientX || (e.touches && e.touches[0].clientX); const clientY = e.clientY || (e.touches && e.touches[0].clientY); startDragRef.current = { x: clientX, y: clientY }; setIsDragging(true); };
    const handlePointerMove = (e) => { if (!isDragging || mode !== 'custom' || !startDragRef.current) return; const clientX = e.clientX || (e.touches && e.touches[0].clientX); const clientY = e.clientY || (e.touches && e.touches[0].clientY); const dx = clientX - startDragRef.current.x; const dy = clientY - startDragRef.current.y; setPos(p => ({ x: Math.max(0, Math.min(100, p.x - dx * 0.2)), y: Math.max(0, Math.min(100, p.y - dy * 0.2)) })); startDragRef.current = { x: clientX, y: clientY }; };
    const handlePointerUp = () => { setIsDragging(false); startDragRef.current = null; };

    const applyCustomWallpaper = () => { changeWallpaper({ bgColor: '#000000', bgImage: `url(${customWallPreview})`, bgSize: mode === 'contain' ? 'contain' : 'cover', bgPos: mode === 'custom' ? `${pos.x}% ${pos.y}%` : 'center', blend: 'normal' }); setCustomWallPreview(null); };

    return (
      <div className="absolute inset-0 z-[100] bg-black/90 flex flex-col items-center justify-center p-4 backdrop-blur-md animate-in fade-in duration-200">
        <h2 className="text-white text-xl font-bold mb-4 drop-shadow-md">Настройка фона</h2>
        <div className={`w-64 h-[400px] border-2 border-white/20 rounded-3xl shadow-2xl relative overflow-hidden bg-black ${mode === 'custom' ? 'cursor-move' : ''}`} onMouseDown={handlePointerDown} onMouseMove={handlePointerMove} onMouseUp={handlePointerUp} onMouseLeave={handlePointerUp} onTouchStart={handlePointerDown} onTouchMove={handlePointerMove} onTouchEnd={handlePointerUp}>
          <div className="absolute inset-0 transition-all duration-75" style={{ backgroundImage: `url(${customWallPreview})`, backgroundSize: mode === 'contain' ? 'contain' : 'cover', backgroundPosition: mode === 'custom' ? `${pos.x}% ${pos.y}%` : 'center', backgroundRepeat: 'no-repeat' }} />
          {mode === 'custom' && <div className="absolute inset-0 bg-black/20 flex flex-col items-center justify-center pointer-events-none"><Move className="text-white/50 w-12 h-12 animate-pulse" /><p className="text-white/70 text-sm mt-2 font-medium bg-black/40 px-3 py-1 rounded-full backdrop-blur-sm">Потяните фото</p></div>}
        </div>
        <div className="flex bg-white/10 rounded-xl p-1 mt-6 shadow-xl backdrop-blur-sm border border-white/10">
          <button onClick={() => setMode('cover')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${mode === 'cover' ? 'bg-white text-black shadow-md' : 'text-white/60 hover:text-white'}`}><Maximize size={16} className="inline-block mr-1 -mt-0.5"/>Заполнить</button>
          <button onClick={() => setMode('contain')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${mode === 'contain' ? 'bg-white text-black shadow-md' : 'text-white/60 hover:text-white'}`}><Minimize size={16} className="inline-block mr-1 -mt-0.5"/>Вместить</button>
          <button onClick={() => { setMode('custom'); setPos({x: 50, y: 50}); }} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${mode === 'custom' ? 'bg-white text-black shadow-md' : 'text-white/60 hover:text-white'}`}><Move size={16} className="inline-block mr-1 -mt-0.5"/>Двигать</button>
        </div>
        <div className="flex gap-4 mt-8 w-64"><button onClick={() => setCustomWallPreview(null)} className="flex-1 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl font-bold transition-all shadow-lg backdrop-blur-sm">Отмена</button><button onClick={applyCustomWallpaper} className="flex-1 py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-xl font-bold transition-all shadow-lg shadow-blue-500/30">Применить</button></div>
      </div>
    );
  };
            ...
            {settingsView === 'main' && (
              <div className="p-3">
                <div className="flex flex-col gap-1">
                  <button onClick={() => setSettingsView('personalization')} className="flex items-center gap-3 p-3 w-full hover:bg-gray-50 active:bg-gray-100 rounded-xl transition-colors text-left">
                    <div className="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center text-purple-500"><Palette size={18} /></div>
                    <span className="font-medium text-[15px] text-gray-800 flex-1">Персонализация</span>
                    <ChevronRight size={18} className="text-gray-400" />
                  </button>
                  <button onClick={() => setSettingsView('notifications')} className="flex items-center gap-3 p-3 w-full hover:bg-gray-50 active:bg-gray-100 rounded-xl transition-colors text-left">
                    <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-500"><Bell size={18} /></div>
                    <span className="font-medium text-[15px] text-gray-800 flex-1">Уведомления</span>
                    <ChevronRight size={18} className="text-gray-400" />
                  </button>
                </div>
              </div>
                ...
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                  ...
                  <div className="grid grid-cols-3 gap-3 max-h-60 overflow-y-auto p-1 scrollbar-hide">
                    {WALLPAPER_PRESETS.map((preset, idx) => (
                      <div 
                        key={idx} 
                        onClick={() => changeWallpaper(preset)} 
                        className={`h-20 rounded-xl cursor-pointer border-2 transition-all hover:scale-105 shadow-sm ${wallpaper.bgImage === preset.bgImage && wallpaper.bgColor === preset.bgColor ? 'border-purple-500' : 'border-transparent'}`} 
                        style={{ backgroundColor: preset.bgColor, backgroundImage: preset.bgImage, backgroundBlendMode: preset.blend, backgroundSize: preset.bgSize }}
                      ></div>
                    ))}
                  </div>
              ...
              <div className="p-5 animate-in slide-in-from-right-4 duration-200">
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                  ...
                  <div className="flex items-center justify-between mb-5">
                    <span className="text-[15px] font-medium text-gray-800">Уведомления</span>
                  ...
                  <div className={`flex flex-col gap-2 mb-5 transition-opacity ${!notificationsEnabled ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                    <div className="flex justify-between items-center text-[13px] text-gray-500 font-medium"><span>Громкость уведомлений</span><span>{Math.round(notificationVolume * 100)}%</span></div>
                    <input type="range" min="0" max="1" step="0.01" value={notificationVolume} onChange={changeNotifVolume} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-500" />
                  </div>
                  <div className="flex flex-col gap-2 pt-4 border-t border-gray-200">
                    <div className="flex justify-between items-center text-[13px] text-gray-500 font-medium"><span>Громкость звонков</span><span>{Math.round(callVolume * 100)}%</span></div>
                    <input type="range" min="0" max="1" step="0.01" value={callVolume} onChange={changeCallVolume} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-500" />
                  </div>
                </div>
              </div>
            )}
      ...
      {callState !== 'idle' && callInfo && (
        <div className={`fixed inset-0 bg-gray-900 flex flex-col justify-between items-center py-10 transition-all duration-500 overflow-hidden ${isCallMinimized ? 'translate-y-full opacity-0 pointer-events-none' : 'translate-y-0 opacity-100 z-[10000]'}`}>
          ...
          <div className="w-full flex justify-center gap-8 md:gap-12 z-20 pb-6">
            ...
            <button onClick={endCall} className="w-16 h-16 md:w-20 md:h-20 bg-red-500 rounded-full flex items-center justify-center hover:bg-red-600 transition-transform active:scale-95 shadow-[0_0_40px_rgba(239,68,68,0.5)]">
